...
class Solution {
    ...
    public long solve(String s, int k, int i, int rest, int prev){
        if(i>=s.length()) return (prev!=10? 1 : 0); // number formed

        if(dp[i][rest][prev]!=null) return dp[i][rest][prev]; // memo
        ...
        for(int d=0; d<=((rest==1)? ub : 9); d++){
            if(rest==1){
                ...
                if(prev==10 || diff<=k){ // check for difference
                    if(d==ub){
                        if(prev==10 && d==0){ // check if number started or not
